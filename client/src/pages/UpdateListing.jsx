...
export default function UpdateListing() {
    ...
    useEffect(() => {
        const fetchListing = async() => {
            //because we sent from app.jsx as listingId in the route
    ...
    const handleImageSubmit = (e) => {
        if(files.length > 0 && files.length + formData.imageUrls.length < 7 ){
            ...
            //set all the urls in the formData
    ...
    const handleRemoveImage = (index) => {
        //only keep the urls with index which is not equal to 'index'
        setFormData({
            ...formData,
    ...
    const handleChange = (e) => {
        //since types of input are different we need to handle differently
    ...
    <main className='p-3 max-w-4xl mx-auto'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                <input onChange={handleChange} type='text' id='name' placeholder='Name' className='border p-3 rounded-lg' maxLength='62' minLength='10' required value={formData.name}/>
                <input onChange={handleChange} type='text' id='description' placeholder='Description' className='border p-3 rounded-lg' required value={formData.description}/>
                <input onChange={handleChange} type='text' id='address' placeholder='Address' className='border p-3 rounded-lg'  required value={formData.address}/>
                <div className='flex gap-6 flex-wrap'>
                    <div className='flex gap-2'>
                        {/* check the box if defaul value is 'sale' */}
                ...
                <div className='flex flex-wrap gap-6'>
                    <div className='flex items-center gap-2'>
                        <input onChange={handleChange} type='number' id='bedrooms' min='1' max='10' required className='p-3 border border-gray-300 rounded-lg' value={formData.bedrooms}/>
                        <p>Beds</p>
                    </div>
                    <div className='flex items-center gap-2'>
                        <input onChange={handleChange} type='number' id='bathrooms' min='1' max='10' required className='p-3 border border-gray-300 rounded-lg' value={formData.bathrooms}/>
                        <p>Baths</p>
                    </div>
                    <div className='flex items-center gap-2'>
                ...
                </p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={index} className='flex justify-between items-center border'>
                            ...
                            <button onClick={() => handleRemoveImage(index)} type='button' className='p-3 text-red-700 uppercase rounded-lg hover:opacity-75'>Delete</button>
                        </div>
                    ))
                }
